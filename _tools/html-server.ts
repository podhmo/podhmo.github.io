...
async function main() {
    ...
    const options_ = parseArgs(Deno.args, {
        string: ["host", "port", "watch-dir"],
        required: ["host", "port", "watch-dir"],
        default: { host: "localhost", port: "8080", "watch-dir": "." },
    });

    const asInteger = moreStrict(options_).integer;
    const options = { ...options_, port: asInteger(options_.port) };
    ...
    // ファイル監視開始
    watchFiles(options["watch-dir"]);
}

// $ deno run --allow-net --allow-read _tools/html-server.ts
if (import.meta.main) {
    await main();
}
